    ...
    def get_queryset(self):
        ...
        if user.role == Role.SUPERADMIN:
            ...
            try:
                ...
                return FitnessLog.objects.filter(player__player_code=profile.player_code, player__academy=user.academy)
            except GuardianProfile.DoesNotExist:
                return FitnessLog.objects.none()
        return FitnessLog.objects.filter(player__academy=user.academy)

    def perform_create(self, serializer):
        if self.request.user.role not in [Role.ADMIN, Role.COACH]:
            raise exceptions.PermissionDenied("Only coaches and admins can register fitness and injury reports.")
        serializer.save()
